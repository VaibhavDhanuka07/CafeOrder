...
export default function MenuItem({ item, category, onAddToCart }) {
  ...
  return (
    <div className="card animate-fadeIn">
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {[100, 200, 300, 500].map(g => (
              <button
                key={g}
                onClick={() => { setQuantity(g); setCustomGrams(''); }}
      ...
      {category.quantityType === 'pieces' && (
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Min. {category.minQuantity} pieces
          </p>
        </div>
      )}
      
      {/* Size Selector for Cheesecakes, Tiramisu, Tea Cakes */}
      {item.sizes && (
        <div className="mb-4">
          <label className="block text-sm font-semibold text-gray-700 mb-2">
            Select Size:
          ...
          <div className="space-y-2">
            {Object.keys(item.sizes).map(size => (
              <button
                ...
                className={`w-full px-4 py-3 rounded-lg font-semibold transition-all flex justify-between items-center ${
                  selectedSize === size
                    ? 'bg-primary text-white'
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                }`}
      ...
      {/* Price Display */}
      <div className="mb-4 py-3 px-4 bg-amber-50 rounded-lg">
        ...
        {category.pricePerUnit && (
          <p className="text-xs text-gray-500 mt-1">
            @ ₹{category.pricePerUnit} per {category.unit}
          </p>
        )}
      </div>
